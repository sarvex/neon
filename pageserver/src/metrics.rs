...
pub fn remove_tenant_metrics(tenant_id: &TenantId) {
    let _ = STORAGE_TIME_SUM_PER_TIMELINE.remove_label_values(&["gc", &tenant_id.to_string(), "-"]);
    let _ =
        STORAGE_TIME_COUNT_PER_TIMELINE.remove_label_values(&["gc", &tenant_id.to_string(), "-"]);
}
